...
public class ProductDTO {
	...
	public ProductDTO(String prodid,
			@NotNull(message = "{Name cannot Be Null}") @Pattern(regexp = "[A-Za-z]+( [A-Za-z]+)*", message = "Name should contain only alphabets and space") String productname,
			@Min(value = 200, message = "Price should not be less than 200") double price,
			@Min(value = 10, message = "Stock should not be less than 10") Integer stock,
			@NotBlank(message = "Cannot be blank") @Size(max = 500, message = "Must be within 500 characters") String description,
			String image, @Min(1) String sellerid, @NotBlank(message = "Category be blank") String category,
			@NotBlank(message = "Sub-Category be blank") String subcategory, @Min(1) Integer productrating) {
		super();
		this.prodid = prodid;
		this.productname = productname;
		this.price = price;
	...
	// Converts Entity into DTO
		...
		// Converts DTO into Entity
			...
			public String toString() {
				return "ProductDTO [prodid=" + prodid + ", productname=" + productname + ", price=" + price + ", stock="
						+ stock + ", description=" + description + ", image=" + image + ", sellerid=" + sellerid
						+ ", category=" + category + ", subcategory=" + subcategory + ", productrating=" + productrating
						+ "]";
			}
